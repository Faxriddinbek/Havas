...
from apps.user.manager.device import DeviceManager
from apps.user.model.users import User


class DeviceTheme(models.TextChoices): #devise rejimi
    DARK = "DARK", "Dark"
    LIGHT = "LIGHT", "Light"
...
class AppVersion(BaseModel): #apni versiyasi
...
class Device(BaseModel):
    ...
    operation_version = models.CharField(max_length=155) # foydalanuvchini devisesini versiyasi
    device_type = models.CharField(
        max_length=7,
        choices=DeviceType.choices,
    ...
    last_login = models.DateTimeField(auto_now=True, db_index=True) #ohirgi martta login qilgan vaqti
    first_login = models.DateTimeField(auto_now_add=True)# birinshi martta login qilgan vaqti
    ...
    is_push_notification = models.BooleanField( # bu foydalanuvchiga habar borish bormasligni aniqlab beradi
        default=True,
        verbose_name="Push Notifications Enabled"
    )
    is_auth_password = models.BooleanField(# qanday turda kirishi (pinkod yoki barmoq orqali)
        default=False,
    ...
    # )
    device_token = models.UUIDField(default=uuid.uuid4, unique=True, db_index=True)# devisega token generatsiya qilishlik
    ...
    firebase_token = models.CharField( #qurilmalarga xabar jo'natish uchun(mobildan olamiz)
        max_length=500,
        unique=True,
        null=True,
        blank=True,
    ...
    # Relationships
    app_version = models.ForeignKey(# bu proect(appni) versiyasi
        AppVersion,
    ...
    user = models.ForeignKey(
        ...
        null=True, # user bo'lishi shart emas
        blank=True, # Bazada saqlanmasligiga ruxsat berish
        related_name="devices",
        db_index=True
    )

    objects = DeviceManager() # bu custom menage ()
